...
class DashboardView(QWidget):
    ...
    def init_ui(self):
        ...
        self.card_cust = MetricCard("TOTAL CUSTOMERS", "0")
        self.card_sold = MetricCard("DEVICES SOLD", "0")
        self.card_active = MetricCard("ACTIVE INSTALLMENTS", "0")
        self.card_completed = MetricCard("COMPLETED INSTALLMENTS", "0")
        self.card_overdue = MetricCard("OVERDUE INSTALLMENTS", "0")
        self.card_outstanding = MetricCard("OUTSTANDING BALANCE", "Rs. 0.00")
        self.card_month = MetricCard("COLLECTIONS (THIS MONTH)", "Rs. 0.00")
        self.card_profit = MetricCard("MONTHLY PROFIT", "Rs. 0.00")
        self.card_revenue = MetricCard("NET REVENUE", "Rs. 0.00")
    ...
    def populate_ui(self, kpis: dict, chart_data: dict):
        ...
        self.card_outstanding.update_value(f"Rs. {kpis['total_outstanding']:,.2f}")
        self.card_month.update_value(f"Rs. {kpis['collections_this_month']:,.2f}")
        self.card_profit.update_value(f"Rs. {kpis['total_profit']:,.2f}")
        self.card_revenue.update_value(f"Rs. {kpis['net_revenue']:,.2f}")
