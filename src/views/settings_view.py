...
class SettingsView(QWidget):
    ...
    def init_ui(self):
        ...
        lbl_decimal = QLabel("Currency Decimal Places")
        lbl_decimal.setStyleSheet("font-weight: bold; margin-top: 6px;")
        form_layout.addWidget(lbl_decimal)

        lbl_decimal_hint = QLabel(
            "Controls how many decimal places are shown in all currency\n"
            "values across the app (invoices, ledgers, dashboards)."
        )
        lbl_decimal_hint.setStyleSheet("font-size: 11px; color: #64748B;")
        lbl_decimal_hint.setWordWrap(True)
        form_layout.addWidget(lbl_decimal_hint)

        self.radio_dec_0 = QRadioButton("No decimals  (Rs. 1,500)")
        self.radio_dec_2 = QRadioButton("2 decimal places  (Rs. 1,500.00)")
        self.radio_dec_2.setChecked(True)  # default

        self.decimal_group = QButtonGroup(self)
        self.decimal_group.addButton(self.radio_dec_0, 0)
        self.decimal_group.addButton(self.radio_dec_2, 2)

        dec_radio_layout = QHBoxLayout()
        dec_radio_layout.setSpacing(20)
        dec_radio_layout.addWidget(self.radio_dec_0)
        dec_radio_layout.addWidget(self.radio_dec_2)
        dec_radio_layout.addStretch()
        form_layout.addLayout(dec_radio_layout)
        ...
        lbl_danger_desc = QLabel(
            "Reset the system database completely. This deletes all customers, "
            "devices, sales, installments, and payment history."
        )
    ...
    def load_settings(self):
        """Loads and pre-populates existing shop details and active settings."""
        try:
            ...
            # Restore decimal places radio selection
            decimal_places = config.get("decimal_places", 2)
            if decimal_places == 0:
                self.radio_dec_0.setChecked(True)
            else:
                self.radio_dec_2.setChecked(True)
    ...
    def save_settings(self, *args):
        ...
        theme   = "light"
        decimal_places = self.decimal_group.checkedId()  # 0 or 2
    ...
    def reset_database(self, *args):
        ...
        confirm = QMessageBox.question(
            ...
            "WARNING: This will completely wipe all customers, devices, sales, "
            "installments, and payment logs.\n\nAre you absolutely sure you want to proceed?",
